...
PRB2CompactQtree::PRB2CompactQtree(vector<Point<uint> > &vp,
		BitSequenceBuilder *bs, BitSequenceBuilder *bb, BitSequenceBuilder *bc,
		int k1, int k2, int F, int max_level_k1, int max_levels_ki) :
		k1_(k1), k2_(k2), F_(F) {
	...
	for (int i = 0; i < depth_; i++) {
		...
		countbits += leaves_per_level_[i];
	}
	printf("Total size C: %lu\n", countbits);
...
void PRB2CompactQtree::get_stats(std::vector<Point<uint> > &vp) {
	struct Node z;
	queue<struct Node> q;
	...
	while (!q.empty()) {
		...
		for (int j = 1; j < children_[z.level] + 1; j++) {
			if (r[j] - r[j - 1] > 0) {
				...
				if (z.level < depth_ - 1) {
					if (r[j] - r[j - 1] <= F_) { //was F
						// printf("level %u (bits) cut here %u %u\n", t.l, t.lo, t.hi);
						//printf("B set bit %u\n", setbits);
						// do nothing :D
						leaves_per_level_[z.level] += r[j] - r[j - 1];
					} else {
...
void PRB2CompactQtree::create(const std::vector<Point<uint> > &vp,
		BitSequenceBuilder *bs, BitSequenceBuilder *bb,
		BitSequenceBuilder *bc) {
	struct Node z;
	queue<struct Node> q;
	size_t r[max_children_ + 1];
	...
	vector<uint*> countbits;
	for (int i = 0; i < depth_; i++) {
		countbits.push_back(new uint[leaves_per_level_[i] / W + 1]);
		fill_n(countbits[i], leaves_per_level_[i] / W + 1, 0); //fill with zeroes
	}
			...
			if (r[j] - r[j - 1] > 0) {
				...
				if (z.level < depth_ - 1) {
					...
					if (r[j] - r[j - 1] <= F_) { //was F
						...
						//printf("set bit C_[%d] %lu\n", z.level, curr_pos[z.level] + r[j]-r[j-1] - 1);
						cds_utils::bitset(countbits[z.level],
								curr_pos[z.level] + r[j] - r[j - 1] - 1);

						for (size_t jj = r[j - 1]; jj < r[j]; jj++) {
							//printf("saving point at pos %lu\n",jj);
							Point<uint> p(vp[jj]);
							uint maxval = nk_[z.level];

							if (false == is_interleaved_) {
								//printf("saving values A at level %d at pos %lu \n",z.level,curr_pos[z.level]);
								...
								if (z.level < levels_k1_ + levels_k2_) {
									...
									if (0
											== rangedim_by_level_[z.level].first) {
...
//                  printf("set bit T_[%d] %lu\n", z.level, curr_nodes[z.level] + j - 1);
	...
	//building bitmaps for C and cleaning
	for (int i = 0; i < depth_; i++) {
		C_.push_back(bc->build(countbits[i], leaves_per_level_[i]));
		delete[] countbits[i];
	}
}

void PRB2CompactQtree::all(Point<uint> p, size_t z, int level,
	...
	if (level == depth_ - 1) {
		...
		if (level == -1) {
			...
			if (B_[level]->access(q) == 1) {
				...
				if (b == 0) {
					lo = 0;
					hi = C_[level]->select1(b + 1) + 1;
				} else {
					lo = C_[level]->select1(b) + 1;
					hi = C_[level]->select1(b + 1) + 1;
				}
...
void PRB2CompactQtree::range(Point<uint> &p, size_t z, int level,
...
void PRB2CompactQtree::stats_space() const {
	size_t treebits = 0;
	size_t leafbits = 0;
	size_t arraysize = 0;
	size_t countbits = 0;
	...
	printf("%10s\t%10s\t%8s\t%10s\t%8s\t", "|B|", "B.1s", "B.1s/|B|", "B.bytes",
			"B.cratio");
	printf("%10s\t%10s\t%8s\t%10s\t%8s\t", "|C|", "C.1s", "C.1s/|C|", "C.bytes",
			"C.cratio");
	printf("%10s\n", "A.bytes");
	for (int i = 0; i < depth_; i++) {
		...
		printf("%10lu\t%10lu\t%8.2f\t%10lu\t%8.2f\t", C_[i]->getLength(),
				C_[i]->countOnes(),
				1.0 * C_[i]->countOnes() / C_[i]->getLength(), C_[i]->getSize(),
				1.0 * C_[i]->getSize() * 8 / C_[i]->getLength());
		size_t A = 0;
...
PRB2CompactQtree::PRB2CompactQtree(ifstream & f) {
...
void PRB2CompactQtree::save(ofstream &f) const {
	uint wr = PRQBLK2_SAV;
...
void PRB2CompactQtree::print_leaves() {
