...
class PRBTest: public ::testing::Test {
protected:
	virtual void SetUp() {
	...
	void AllPointsTester(int k1 = 2, int k2 = 2, int max_level_k1 = 0,
		...
		printf("checking k1=%d k2=%d lk1=%d lki=%d\n", k1, k2, max_level_k1,
				max_level_ki);
		PRBCompactQtree a(vp_, new BitSequenceBuilderRG(20),
				new BitSequenceBuilderRG(20), k1, k2, max_level_k1,
				max_level_ki);
...
TEST_F(PRBTest, AllPointsSimple) {
	AllPointsTester(2, 2, 0, 0);
}

TEST_F(PRBTest, K1Equals4) {
	int k1 = 4;
	int k2 = 2;
...
TEST_F(PRBTest, LoadSaveBasic) {
	LoadSave();
}

TEST_F(PRBTest, LoadSaveAllMods) {
	int k1 = 4;
	int k2 = 2;
	Point<uint> pmax_dim(max(vp_));
	uint maxval;
