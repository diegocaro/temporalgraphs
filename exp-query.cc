...
int main(int argc, char ** argv) {
	...
	if (argc < 5) {
		fprintf(stderr,
				"Usage: %s <graphfile> <queryfile> <typequery> (<time> | <initime> <endtime>)\n",
				argv[0]);
	...
	while (EOF != getQuery(queryFile, &query)) {
		...
		case EDGE_WEAK: {
			...
			gotres = index->edge_weak(query.row, query.column,
		...
		case EDGE_STRONG: {
			//gotres = findEdgeInterval(tree, query.row, query.column, timeIntervalStart, timeIntervalEnd, 1);
			gotres = index->edge_strong(query.row, query.column,
					timeIntervalStart, timeIntervalEnd);
		...
		case DIRECT_NEIGHBORS_WEAK: {
			...
			index->direct_weak(query.row, timeIntervalStart, timeIntervalEnd,
					gotreslist);
			break;
		}
		case REVERSE_NEIGHBORS_WEAK: {
			//get_reverse_weak(gotreslist, &index, query.row, timeIntervalStart, timeIntervalEnd);
			index->reverse_weak(query.row, timeIntervalStart, timeIntervalEnd,
					gotreslist);
			break;
		}
		case DIRECT_NEIGHBORS_STRONG: {
			//get_neighbors_strong(gotreslist, &index, query.row, timeIntervalStart, timeIntervalEnd);
			index->direct_strong(query.row, timeIntervalStart, timeIntervalEnd,
					gotreslist);
			break;
		}
		case REVERSE_NEIGHBORS_STRONG: {
			//get_reverse_strong(gotreslist, &index, query.row, timeIntervalStart, timeIntervalEnd);
			index->reverse_strong(query.row, timeIntervalStart, timeIntervalEnd,
					gotreslist);
			break;
		}
...
//                      gotres = findRange(tree, 0, tree->nNodesReal, 0, tree->nNodesReal, time)[0][0];
		...
		case REVERSE_NEIGHBORS:
		case SNAPSHOT: {
			printf("%s\t%u\t%ld\t%u\t%u\t%u\n", fileNameIndex, query.xvalue,
					endClockTime(), *gotreslist, typeQuery, timePoint);
			break;
		}
		case EDGE_WEAK:
		case EDGE_STRONG:
		...
		case REVERSE_NEIGHBORS_STRONG: {
			printf("%s\t%u\t%ld\t%u\t%u\t%u\t%u\n", fileNameIndex, query.xvalue,
					endClockTime(), *gotreslist, typeQuery, timeIntervalStart,
					timeIntervalEnd);
			break;
		}
		}
