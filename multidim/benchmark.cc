	...
	while (1) {
		...
		case EDGE_STRONG: {
			res = fscanf(queryFile, "%d %d %d %d\n", &query->row,
					&query->column, &query->initime, &query->endtime);
			break;
		}
		...
		if (query->type == EDGE || query->type == EDGE_NEXT
				|| query->type == EDGE_STRONG || query->type == EDGE_WEAK
				|| query->type == SNAPSHOT || query->type == CHANGE_POINT
				|| query->type == CHANGE_INTERVAL
				|| query->type == ACTIVED_POINT
				|| query->type == ACTIVED_INTERVAL
				|| query->type == DEACTIVED_POINT
				|| query->type == DEACTIVED_INTERVAL)
			res = fscanf(queryFile, "%d\n", &query->expectednres);
		else {
			res = fscanf(queryFile, "%d", &query->expectednres);
			uint i;
			query->expectedres = (unsigned int *) malloc(
					(1 + query->expectednres) * sizeof(unsigned int));
...
void printQuery(TimeQuery q) {
	printf("%d %d %d %d %d %d %d\n", q.type, q.row, q.column, q.time, q.initime,
			q.endtime, q.expectednres);
}
	...
	for (i = 0; i < nqueries; i++) {
		*gotreslist = 0;
		...
		case EDGE_WEAK: {
			...
			gotres = index->edge_weak(query.row, query.column, query.initime,
					query.endtime);
			*gotreslist = gotres;
			break;
		}
		case EDGE_STRONG: {
			//gotres = findEdgeInterval(tree, query.row, query.column, query.initime, query.endtime, 1);
			gotres = index->edge_strong(query.row, query.column, query.initime,
					query.endtime);
			*gotreslist = gotres;
			break;
		}
		...
		case DIRECT_NEIGHBORS_WEAK: {
			...
			index->direct_weak(query.row, query.initime, query.endtime + 1,
					gotreslist);
			break;
		}
		case REVERSE_NEIGHBORS_WEAK: {
			//get_reverse_weak(gotreslist, &index, query.row, query.initime, query.endtime);
			index->reverse_weak(query.row, query.initime, query.endtime + 1,
					gotreslist);
			break;
		}
		case DIRECT_NEIGHBORS_STRONG: {
			//get_neighbors_strong(gotreslist, &index, query.row, query.initime, query.endtime);
			index->direct_strong(query.row, query.initime, query.endtime + 1,
					gotreslist);
			break;
		}
		case REVERSE_NEIGHBORS_STRONG: {
//                        get_reverse_strong(gotreslist, &index, query.row, query.initime, query.endtime);
			index->reverse_strong(query.row, query.initime, query.endtime + 1,
					gotreslist);
			break;
		}
...
//                //Comentar para medir tiempos:
		if (CHECK_RESULTS) {
			...
			if (savegotFile) {
				...
				if (query.type == EDGE || query.type == EDGE_NEXT
						|| query.type == EDGE_WEAK || query.type == EDGE_STRONG
						|| query.type == SNAPSHOT || query.type == CHANGE_POINT
						|| query.type == CHANGE_INTERVAL
						|| query.type == ACTIVED_POINT
						|| query.type == ACTIVED_INTERVAL
			...
			if (failcompare) {
				printf(
						"query queryType=%d, row=%d, column=%d, time=%d, initime=%d, endtime=%d, expectedres=%d\n",
						query.type, query.row, query.column, query.time,
						query.initime, query.endtime, query.expectednres);
				...
				if (!(query.type == EDGE || query.type == EDGE_NEXT
						...
						|| query.type == DEACTIVED_INTERVAL)) {
					printf("expected: ");
					print_arraysort(query.expectedres);
					printf("got     : ");
					print_arraysort(gotreslist);
				}
				exit(1);
			}
			totalres += gotres;
...
//	               difftime/nqueries, difftime/totalres);
	...
	printf("%s\t%s\t%ld\t%d\t%d\t%lf\t%lf\n", fileName, fileQueries, microsecs,
			nqueries, totalres, (double) microsecs / nqueries,
